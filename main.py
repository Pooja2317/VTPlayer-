...
windows = tk.Tk()
windows.title('VTPlayer - Music Player')
...
def updatetitle(title):
    ...
    global filefound
    if filefound != True:
        title = f'VTPLayer - Add songs to playlist first! ' + title
        root.title(title)
        root.update()
        return
    title = f'VTPLayer - Playing:   ' + title
    windows.title(title)
    windows.update()
...
def aboutwindow():
    ...
    lbl1 = Label(infoframe, text='VTPLayer - Music Player', font=('AdobeClean-Bold', 13))
    ...
    about = Label(aboutframe, text='VTPlayer is a Stylish, Powerful and  Fast Music Player  with  elegant design. \n' \
    ...
    features = Label(aboutframe, text='Currently VTPlayer fully Supports (Features): \n\n' \
    ...
    footerlbl = Label(footer, text='VTPlayer player is a Music  Player  for  Windows. Send  me  the feedbacks,\n' \
                                   'bug-reports and suggestions about VTPlayer to:', justify=LEFT)
    footerlbl.place(x=0, y=0)
    emaillbl = Label(footer, text='', fg="blue", cursor="hand2")
...
def licenses():
    ...
    height = 480
    win = tk.Toplevel()
    win.wm_title("VTPLayer - View Licenses")
    screen_width = windows.winfo_screenwidth()
    screen_height = windows.winfo_screenheight()
...
text1 = Label(addlibrary, text="LIBRARY", font=('AdobeClean-Bold', 13), fg=textColor,
              bg=leftFrameColor)  # HEADING: LIBRARY fg='#686868'
...
btn = Button(addlibrary, text="Add library", font=('AdobeClean-REGULAR', 12), borderwidth=0, fg=textColor,
             bg=leftFrameColor, command=addlibFolder)  # ADD FOLDER TO LIBRARY
btn.place(x=55, y=70)
btn1 = Button(addlibrary, text="Add Songs", font=('AdobeClean-REGULAR', 12), borderwidth=0, fg=textColor,
              bg=leftFrameColor, command=addSongs)  # ADD SONGS TO LIBRARY
btn1.place(x=55, y=95)
btn2 = Button(addlibrary, text="Remove current song", font=('AdobeClean-REGULAR', 12), borderwidth=0, fg=textColor,
              bg=leftFrameColor, command=lambda: removeSongs('ONE'))  # Removes current song from LIBRARY
btn2.place(x=55, y=120)
btn3 = Button(addlibrary, text="Remove all Songs", font=('AdobeClean-REGULAR', 12), borderwidth=0, fg=textColor,
              bg=leftFrameColor, command=lambda: removeSongs('ALL'))  # Removes all songs from LIBRARY
btn3.place(x=55, y=145)
text3 = Label(addlibrary, text="USER CONTROL", font=('AdobeClean-Bold', 13), fg=textColor,
              bg=leftFrameColor)  # HEADING: DETAILS
text3.place(x=10, y=180)
aboutbtn = Button(addlibrary, text="About", font=('AdobeClean-REGULAR', 12), borderwidth=0, fg=textColor,
                  bg=leftFrameColor, command=aboutwindow)
aboutbtn.place(x=55, y=210)
licensebtn = Button(addlibrary, text='View License', font=('AdobeClean-REGULAR', 12), borderwidth=0, fg=textColor,
                    bg=leftFrameColor, command=licenses)
